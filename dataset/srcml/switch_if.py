...
# start to transform
def trans_tree(e, line_num):
    global flag
    global flag_switch
    # get all swtich
    switch_elems = get_switch(e)
    for switch_elem in switch_elems:
        switch_line_number = get_line_number(switch_elem)
        if switch_line_number != line_num+2:
            continue
        # judge whether switch can be transformed
        flag_switch = True
...
# get the line number of the switch statement
def get_line_number(switch_elem):
    return switch_elem.sourceline
