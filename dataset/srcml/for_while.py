...
def trans_tree(e, line_num, ignore_list=[], instances=None):
    ...
    for item in for_stmts:
        for for_stmt in item:
            for_line_number = get_line_number(for_stmt)
            if for_line_number != line_num+2:
                continue
...
# get the line number of the for statement
def get_line_number(for_elem):
    return for_elem.sourceline
