...
# start to transform
def trans_tree(e, line_num, ignore_list=[], instances=None):
    global flag
    flag = False
    # get all while
    ...
    if len(while_elems) > 0:
        for item in while_elems:
            for while_elem in item:
                ...
                '''
                while_line_number = get_line_number(while_elem)
                if while_line_number != line_num+2:
                    continue
...
# get the line_number of the while statement
def get_line_number(while_elem):
    return while_elem.sourceline
...
def program_transform(program_path, line_number):
    # get the dir of the program_path
    program_path = os.path.abspath(program_path)
    e = init_parse(program_path)
    trans_tree(e, line_number)
    xml_file = os.path.join(os.path.dirname(program_path), "tmp_changed.xml")
    save_tree_to_file(doc, xml_file)
