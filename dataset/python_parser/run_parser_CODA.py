...
path = '/data/sub3/dxh/Adv/dataset/python_parser/parser_folder/my-languages.so'
...
def get_example_coda(code, tgt_word, substitute, lang):
    parser = parsers[lang]
    code = code.replace("\\n", "\n")
    parser = parsers[lang]
    tree = parser[0].parse(bytes(code, 'utf8'))
...
def get_identifiers_coda(code, lang, only_code_tokens=False):
    code = code.replace("\\n", "\n")
    try:
        code = remove_comments_and_docstrings(code, lang)
    except:
