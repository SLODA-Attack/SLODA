...
from run_parser import get_example, get_example_batch
import torch
...
def clear_var(str_list):
    new_str_list = []
    for index, str in enumerate(str_list):
        new_str_list.append(str.strip())
    regex = r"^[a-zA-Z_][a-zA-Z0-9_]*$"
    valid_var_names = [s for s in new_str_list if re.match(regex, s)]
    return valid_var_names
