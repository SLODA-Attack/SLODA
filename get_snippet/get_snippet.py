...
sys.path.append('../GraphCodeBERT/code/')   # CodeBERT, UniXcoder, GraphCodeBERT, EPVD, ReGVD
...
def main():
    ...
    query_times = 0
    attacker = ALERT_Attacker(args, ptm_model, model, tokenizer, tokenizer_mlm, 1, 0)#attacker = ALERT_Attacker(args, ptm_model, model, tokenizer, tokenizer_mlm, use_bpe=1, threshold_pred_score=0)
    ...
    for index, example in enumerate(eval_dataset):
        ...
        if orig_pred != label:
            if orig_pred == 0:
                ...
                results = attacker.get_code_snippet_one(code, label, "c", example)
                if len(results) == 0:
                    continue
                else:
                    with open('one2zero.jsonl', 'a') as file:
                        for (snippet, var, params) in results:
                            item = {"snippets": snippet, "var": var, "params": params}
                ...
                if len(results) == 0:
                    ...
                    with open('zero2one.jsonl', 'a') as file:
                        for (snippet, var, params) in results:
                            item = {"snippets": snippet, "var": var, "params": params}
                            file.write(json.dumps(item) + '\n')
            continue
