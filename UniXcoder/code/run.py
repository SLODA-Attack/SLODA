...
def test(args, model, tokenizer, data_file):
    ...
    return result
def test1(args, model, tokenizer, data_file):
    """ Evaluate the model """
    eval_dataset = TextDataset(tokenizer, args, data_file)
    eval_sampler = SequentialSampler(eval_dataset)
    eval_dataloader = DataLoader(eval_dataset, sampler=eval_sampler, batch_size=args.eval_batch_size, num_workers=4)
...
def main():
    ...
    parser.add_argument('--validation_metric', type=str, default='acc',
    ...
    # Evaluation
    if args.do_eval:
        output_dir = os.path.join(args.output_dir, 'checkpoint-best-acc/model.bin')
    ...
    if args.do_test:
        ...
        model_to_load = model.module if hasattr(model, 'module') else model
        model_to_load.load_state_dict(torch.load(output_dir))
        result = test1(args, model, tokenizer, args.test_data_file)
