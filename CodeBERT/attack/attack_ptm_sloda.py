...
from ptm_attacker import SLODA_Attacker, get_importance_score
...
def main():
    ...
    attacker = TAA_Attacker(args, ptm_model, model, tokenizer)
    ...
    for index, example in enumerate(eval_dataset):
        if index >= 2612:
            ...
            is_success, adv_codes, new_code, query_times1 = attacker.code_trans_attack(code, label, orig_prob,
                                                                                       orig_pred)
            if is_success == 1:
            ...
            is_success, adv_code, query_times2 = attacker.taa_attack(label, id2token, code_lines, new_code,
                                                                     query_times1)
